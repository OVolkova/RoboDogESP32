...
bool AudioGeneratorWAV::ReadWAVInfo()
{
  ...
  Serial.println("[WAV] Starting ReadWAVInfo()");
  Serial.printf("[WAV] File position: %u\n", file->getPos());
  Serial.printf("[WAV] File size: %u\n", file->getSize());
  Serial.printf("[WAV] File is open: %d\n", file->isOpen());

  // Header == "RIFF"
  Serial.println("[WAV] Reading RIFF header (expecting 0x46464952 = 'RIFF')...");
  if (!ReadU32(&u32)) {
    Serial.printf_P(PSTR("AudioGeneratorWAV::ReadWAVInfo: failed to read WAV data\n"));
    return false;
  };
  if (u32 != 0x46464952) {
    Serial.printf("[WAV] ERROR: Expected 0x46464952 ('RIFF'), got 0x%08X\n", u32);
    uint8_t* bytes = (uint8_t*)&u32;
    Serial.printf("[WAV] As ASCII: '%c%c%c%c'\n",
                  (bytes[0] >= 32 && bytes[0] < 127) ? bytes[0] : '.',
                  (bytes[1] >= 32 && bytes[1] < 127) ? bytes[1] : '.',
                  (bytes[2] >= 32 && bytes[2] < 127) ? bytes[2] : '.',
                  (bytes[3] >= 32 && bytes[3] < 127) ? bytes[3] : '.');
    Serial.printf_P(PSTR("AudioGeneratorWAV::ReadWAVInfo: cannot read WAV, invalid RIFF header\n"));
    return false;
  }

  // Skip ChunkSize
  Serial.println("[WAV] Skip ChunkSize...");
  if (!ReadU32(&u32)) {
    Serial.printf_P(PSTR("AudioGeneratorWAV::ReadWAVInfo: failed to read WAV data\n"));
    return false;
  };
  ...
  // Format == "WAVE"
  Serial.println("[WAV] Format == \"WAVE\"...");
  if (!ReadU32(&u32)) {
    Serial.printf_P(PSTR("AudioGeneratorWAV::ReadWAVInfo: failed to read WAV data\n"));
    return false;
  };
  ...
  Serial.println("[WAV] Searching for 'fmt ' chunk...");
  while (1) {
    if (!ReadU32(&u32)) {
      Serial.printf_P(PSTR("AudioGeneratorWAV::ReadWAVInfo: failed to read WAV data\n"));
      return false;
  ...
  // subchunk size
  Serial.println("[WAV] Reading subchunk size...");
  if (!ReadU32(&u32)) {
    Serial.printf_P(PSTR("AudioGeneratorWAV::ReadWAVInfo: failed to read WAV data\n"));
    return false;
  };
  ...
  // AudioFormat
  Serial.println("[WAV] Reading AudioFormat...");
  if (!ReadU16(&u16)) {
    Serial.printf_P(PSTR("AudioGeneratorWAV::ReadWAVInfo: failed to read WAV data\n"));
    return false;
  };
  ...
  // NumChannels
  Serial.println("[WAV] Reading NumChannels...");
  if (!ReadU16(&channels)) {
    Serial.printf_P(PSTR("AudioGeneratorWAV::ReadWAVInfo: failed to read WAV data\n"));
    return false;
  };
  ...
  // SampleRate
  Serial.println("[WAV] Reading SampleRate...");
  ...
  Serial.println("[WAV] Skipping ByteRate and BlockAlign...");
  if (!ReadU32(&u32)) {
    Serial.printf_P(PSTR("AudioGeneratorWAV::ReadWAVInfo: failed to read WAV data\n"));
    return false;
  };
  ...
  // Bits per sample
  Serial.println("[WAV] Reading BitsPerSample...");
  ...
  Serial.println("[WAV] Skipping extra header bytes...");
  while (toSkip) {
    uint8_t ign;
    if (!ReadU8(&ign)) {
      Serial.printf_P(PSTR("AudioGeneratorWAV::ReadWAVInfo: failed to read WAV data\n"));
  ...
  do {
    // id == "data"
    Serial.println("[WAV] Searching for 'data' chunk...");
    if (!ReadU32(&u32)) {
      Serial.printf_P(PSTR("AudioGeneratorWAV::ReadWAVInfo: failed to read WAV data\n"));
      return false;
    };
    ...
    Serial.println("[WAV] Skipping chunk size...");
    if (!ReadU32(&u32)) {
      Serial.printf_P(PSTR("AudioGeneratorWAV::ReadWAVInfo: failed to read WAV data\n"));
      return false;
    };
  ...
  // is file open?
  Serial.println("[WAV] Checking if file is open...");
  if (!file->isOpen()) {
  ...
  Serial.println("[WAV] Skip size, read until end of file...");
  if (!ReadU32(&u32)) {
    Serial.printf_P(PSTR("AudioGeneratorWAV::ReadWAVInfo: failed to read WAV data\n"));
    return false;
  };
  ...
  Serial.println("[WAV] Setting up read buffer...");
