...
bool AudioOutputI2S::SetRate(int hz)
{
  ...
  Serial.printf("[I2S] SetRate called with %d Hz (i2sOn=%d)\n", hz, i2sOn);
  this->hertz = hz;
  if (i2sOn)
  {
  #ifdef ESP32
      int adjusted = AdjustI2SRate(hz);
      Serial.printf("[I2S] Calling i2s_set_sample_rates with adjusted rate %d Hz\n", adjusted);
      i2s_set_sample_rates((i2s_port_t)portNo, adjusted);
...
bool AudioOutputI2S::begin(bool txDAC)
{
    ...
    if (!i2sOn)
    {
      ...
      i2s_config_t i2s_config_dac = {
          .mode = mode,
          .sample_rate = (uint32_t)this->hertz,  // Use stored sample rate instead of hardcoded 44100
